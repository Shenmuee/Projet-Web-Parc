...
$biomes = [];
$sql_biomes = "SELECT * FROM biomes";
$stmt_biomes = $conn->query($sql_biomes);
while ($row = $stmt_biomes->fetch(PDO::FETCH_ASSOC)) {
    $biomes[$row['id']] = $row;
}

$enclosures = [];
$sql_enclosures = "SELECT * FROM enclosures WHERE maintenance = 0";
$stmt_enclosures = $conn->query($sql_enclosures);
while ($row = $stmt_enclosures->fetch(PDO::FETCH_ASSOC)) {
    $enclosures[$row['id_biomes']][] = $row;
}

$animals = [];
$sql_animals = "SELECT animals.name, relation_enclos_animals.id_enclos 
                FROM animals
